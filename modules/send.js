...
class Send {
	 ...
	 * @param { HtbEmbeds } embedGenerator 
	 */
	constructor(client = null, embedGenerator = null) {
		this.CLIENT = client
		this.EGI = embedGenerator
		this.PASSTHRU = false
	...
	async messagePrimaryDiscordAdmin(title, description) {
		let adminDM = await this.CLIENT.guilds.cache.get(process.env.DISCORD_GUILD_ID.toString()).members.fetch(`${JSON.parse(process.env.ADMIN_DISCORD_IDS)[0]}`)
		// let t = this.CLIENT.users.fetch(`${JSON.parse(process.env.ADMIN_DISCORD_IDS)[0]}`, false).then((user) => {
		// 	user.send("heloo")
		// })
		// console.log(t)
		await adminDM.send(this.EGI.TEAM_INFO_BASE.setTitle(title).setDescription(description))
	}
